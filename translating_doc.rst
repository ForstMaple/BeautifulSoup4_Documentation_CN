...
`Beautiful Soup <http://www.crummy.com/software/BeautifulSoup/>`_ 是一个用来提取HTML和XML文件中的数据的Python库。
它可以与你熟悉的解析器（parser）协作，在文档树中实现符合习惯的移动、搜索和编辑操作，常常能节省使用者几小时甚至于几天的时间。

这篇文档结合各种例子说明了Beautiful Soup 4的所有主要特性。这篇文档将为你展示它适用于什么任务，它是如何工作的，如何使用它，以及它没有按照你的预想工作时该怎么办。
...
如果你对Beautiful Soup有任何疑问或遇到了任何问题，
`你可以发送邮件到讨论组
<https://groups.google.com/forum/?fromgroups#!forum/beautifulsoup>`_. 
如果你的问题涉及对HTML文档的解析，请务必提供 :ref:`diagnose()函数<diagnose>` 对那篇文档的输出。

快速使用指南
============

下面这段HTML代码将作为例子在本篇文档中多次用到，它是《爱丽丝梦游仙境》的一个片段::
...
使用Beautiful Soup解析这篇“Three sisters”文档会生成一个 ``BeautifulSoup`` 对象，他将整篇文档表示为了一个嵌套的数据结构::
...
如果这看上去是你需要的功能，请继续往下阅读。

安装Beautiful Soup
=========================

如果你使用的是新版的Debian或Ubuntu系统，你可以通过你系统的软件包管理器来安装Beautiful Soup:

:kbd:`$ apt-get install python-bs4` (对应Python 2)

:kbd:`$ apt-get install python3-bs4` (对应Python 3)

Beautiful Soup 4也通过PyPi发布了，因此如果你系统的包管理器无法安装Beautiful Soup 4的话，你可以使用 ``easy_install`` 或 ``pip`` 来安装。
这个包的名字是 ``beautifulsoup4`` ，它在Python 2和Python 3中都可以运行。
请确保你使用了对应你Python版本的 ``pip`` 或 ``easy_install`` （如果你使用的是Python 3的话，对应的的命令可能是 ``pip3`` 和 ``easy_install3`` ）。

:kbd:`$ easy_install beautifulsoup4`

:kbd:`$ pip install beautifulsoup4`

（还有个  ``BeaufitulSoup``  包对应的是上一个主要版本 `Beautiful Soup 3`_  ，并不是你需要的。
因为许多软件仍在使用 `Beautiful Soup 3`_ ，所以它还被保留着。但如果你是写新的代码，你应该安装 ``beautifulsoup4`` 。）

如果你没有安装 ``easy_install`` 或 ``pip`` ，你可以 `下载 Beautiful Soup 4 的源码 <http://www.crummy.com/software/BeautifulSoup/download/4.x/>`_ 。
并使用 ``setup.py`` 来安装它。

:kbd:`$ python setup.py install`

你可以下载源码，并将其中的 ``bs4`` 目录拷贝到你的程序目录中，这样就可以在不安装的情况下使用Beautiful Soup。

Beautiful Soup是用Python 2.7和Python 3.8开发的，但它应该也能在其他更新的版本的Python上运行。

安装后的问题
---------------------------

Beaufitul Soup是以Python 2的代码打包的。当你在Python 3下安装它时，他会自动转换成Python 3的代码。
如果你不是“安装”的它，这些代码就就不会自动转换。已经有了一些在Windows机器上安装了错误版本的报告。

如果你遇到 ``ImportError`` "No module named HTMLParser"， 问题在于你在用Python 3运行Python 2的代码。

如果你遇到 ``ImportError`` "No module named html.parser", 问题在于你在用Python 2运行Python 3的代码。

在两种情况下，你最好的选择是删除已安装的所有Beautiful Soup的文件（包括你解压源码的所有目录），然后尝试重新安装。

如果你在 ``ROOT_TAG_NAME = u'[document]'`` 这一行遇到了 ``SyntaxError`` "Invalid syntax" ，
你则需要把Python 2的代码转换为Python 3的代码。

你可以通过安装这个包的方式来完成:

:kbd:`$ python3 setup.py install`

或者在 ``bs4`` 目录下手动运行 ``2to3`` 转换脚本：
...
Beautiful Soup支持Python标准库中自带的HTML解析器，它同时也支持一些第三方的解析器，
其中一个是 `lxml解析器 <http://lxml.de/>`_ 。根据你使用的系统不同，你应该可以通过以下的某一种方式来安装lxml：
...
另一个选择是纯Python实现的 `html5lib解析器 <http://code.google.com/p/html5lib/>`_ ，
它用与浏览器相同的方式来解析HTML文档。根据你使用的系统不同，你应该可以通过以下的某一种方式来安装html5lib：
...
| 解析器                                   | 使用方法                                             | 优点                                              | 缺点                                    |
+------------------------------------------+------------------------------------------------------+---------------------------------------------------+-----------------------------------------+
| Python标准库的html.parser                | ``BeautifulSoup(markup, "html.parser")``             | * 无需另外安装                                    | * 没有lxml解析速度快                    |
|                                          |                                                      | * 解析速度尚可                                    | * 没有html5lib容错能力强                |
|                                          |                                                      | * 容错能力强（Python 2.7.3 和3.2.2以后版本)       |                                         |
|                                          |                                                      |                                                   |                                         |
+------------------------------------------+------------------------------------------------------+---------------------------------------------------+-----------------------------------------+
| lxml库的HTML解析器                       | ``BeautifulSoup(markup, "lxml")``                    | * 解析速度很快                                    | * 依赖额外的C语言扩展                   |
|                                          |                                                      | * 容错能力强                                      |                                         |
+------------------------------------------+------------------------------------------------------+---------------------------------------------------+-----------------------------------------+
| lxml库的XML解析器                        | ``BeautifulSoup(markup, "lxml-xml")``                | * 解析速度很快                                    | * 依赖额外的C语言扩展                   |
|                                          | ``BeautifulSoup(markup, "xml")``                     | * 目前唯一支持的XML解析器                         |                                         |
|                                          |                                                      |                                                   |                                         |
+------------------------------------------+------------------------------------------------------+---------------------------------------------------+-----------------------------------------+
| html5lib                                 | ``BeautifulSoup(markup, "html5lib")``                | * 容错能力非常强                                  | * 解析速度很慢                          |
|                                          |                                                      | * 与浏览器解析方式相同                            | * 依赖额外的Python扩展                  |
|                                          |                                                      | * 生成HTML5                                       |                                         |
|                                          |                                                      |                                                   |                                         |
+------------------------------------------+------------------------------------------------------+---------------------------------------------------+-----------------------------------------+

如果可以的话，我建议你安装并使用lxml以获得更快的解析速度。如果你在使用相对较老版本的Python（2.7.3或3.2.2以前的版本），
那么安装lxml或html5lib解析器就是非常必要的，因为那些旧版本里内置的HTML解析器并不好用。

请注意，如果一个文档自己的格式有问题的话，不同的解析器可能会得到不同的文档树。
详见 `不同解析器的区别`_。

生成BeautifulSoup对象
=========================================

想要解析一个文档，你只需要将它传入 ``BeautifulSoup()`` 。
你可以传入一个字符串，也可以传入一个文件句柄::
...
首先，文档被转换为Unicode，所有的HTML实体都被转换为Unicode字符::

 print(BeautifulSoup("<html><head></head><body>Sacr&eacute; bleu!</body></html>", "html.parser"))
 # <html><head></head><body>Sacré bleu!</body></html>

Beautiful Soup随后会使用最合适的解析器来解析这篇文档。
Beautiful Soup会默认使用HTML解析器，除非你明确指定使用XML解析器。 (参见 `解析XML文档`_。)

对象的种类
================

Beautiful Soup 将一个复杂的HTML文档转换成了一个由不同Python对象组成的树形结构。
你只需要用到其中的四种对象：``Tag``， ``NavigableString``， ``BeautifulSoup``
和 ``Comment``。
...
``Tag`` 对象对应的就是原始XML或HTML文档中的tag（标签）::
...
tag对象有许多属性和方法，其中的大多数我会在 `在文档树中移动`_ 和 `对文档树进行搜索`_ 部分讲解。 
这里我们先讲解tag对象最重要的两个属性：name和attributes。

.. note:: 如未另外说明，本文档中的tag对象都是指Beautiful Soup生成的Python对象，你可以通过调用它的属性来获取原始XML或HTML文档中的tag的标签名（name)和属性（attributes）等。
          如果你对文档中所指的对象感到困惑，参考示例代码或许会有所帮助。

Name
^^^^

每一个tag都有name属性，我们可以通过 ``.name`` 来获得::

 tag.name
 # 'b'

如果你改变了一个tag的name，这一改变会反映在任何由Beautiful Soup生成的HTML标记（markup）中::
...
Attributes
^^^^^^^^^^
一个tag可以有任何数量的attributes [#]_ 。例如 ``<b id="boldest">`` 这个tag有一个值为 "boldest" 的 "id" attribute。
你可以把tag的attributes视作Python的字典（dictionary）来访问::

 tag = BeautifulSoup('<b id="boldest">bold</b>', 'html.parser').b
 tag['id']
 # 'boldest'

你可以通过 ``.attrs`` 来访问上面提到的字典::

 tag.attrs
 # {'id': 'boldest'}

你可以增加、移除和编辑一个tag的attributes，这也是通过字典的操作实现的::
...
HTML 4定义了一些可以有多个值的attributes。HTML 5中删掉了一些，但又定义了一些新的多值attributes。
最常见的多值attribute是 ``class`` (一个tag可以有多个CSS class)，除此之外还有 ``rel`` 、 ``rev`` 、 ``accept-charset`` 、
``headers`` 和 ``accesskey`` 等。Beautiful Soup将多值attribtues的值以一个列表（list）来表示::
...
你可以向 ``BeautifulSoup()`` 传入 ``multi_valued_attributes=None`` 使多值attribute作为一个值返回::

 no_list_soup = BeautifulSoup('<p class="body strikeout"></p>', 'html.parser', multi_valued_attributes=None)
 no_list_soup.p['class']
 # 'body strikeout'

如果一个attribute “看起来” 有多个值，但它又不是任何HTML标准中定义的多值attribute，Beautiful Soup则不会对它进行处理，而是返回一个字符串::

 id_soup = BeautifulSoup('<p id="my id"></p>', 'html.parser')
 id_soup.p['id']
 # 'my id'

一个attribute的多个值在标记（markup）中会被合并成类似字符串的形式::
...
你可以使用 ``get_attribute_list`` 始终以列表方式返回attribute的值，无论这个attribute有几个值::

 id_soup.p.get_attribute_list('id')
 # ["my id"]
 
如果你选择XML解析器来解析文档，则不会出现多值attribute（因为XML本来是不允许一个属性有多个值的，XML的元素可以）::

 xml_soup = BeautifulSoup('<p class="body strikeout"></p>', 'xml')
 xml_soup.p['class']
 # 'body strikeout'

但是你可以传入 ``multi_valued_attributes`` 参数来像HTML中一样处理多值attribute::
...
你应该用不上这个功能。如果你真的需要的话，请参考默认值，它们参照的是HTML规范中描述的规则::
...
``NavigableString`` 对应的是tag中的小段文本的字符串::
...
``NavigableString`` 就像Python中的Unicode字符串，不过它还支持一些 `在文档树中移动`_ 和 `对文档树进行搜索`_ 中描述的特性。
你可以用Python 2的 ``unicode()`` 函数或Python 3的 ``str()`` 函数将一个 ``NavigableString`` 转换为Unicode字符串::
...
你不能就地编辑（edit in place） ``NavigableString`` ，但是你可以使用 :ref:`replace_with()` 方法来替换字符串::

 tag.string.replace_with("No longer bold")
 tag
 # <b class="boldest">No longer bold</b>

``NavigableString`` 支持 `在文档树中移动`_ 和 `对文档树进行搜索`_ 中描述的绝大多数特性，但不是所有特性。
具体来说，因为字符串中没有办法包含其他对象（tag中即可以包含字符串，又可以包含另一个tag），
所以 ``NavigableString`` 不支持 ``.contents`` 或 ``.string`` 属性，也不支持 ``.find()`` 方法。  

如果你想要在Beautiful Soup以外使用一个 ``NavigableString`` ，
你一定要用 ``unicode()`` 或 ``str()`` 把它转换为普通的Unicode字符串。
如果你不这么做，它仍然会是指向整个Beautiful Soup文档树的一个引用，即使你不再用Beautiful Soup了，这会浪费很多内存。

``BeautifulSoup`` 对象
----------------------------

``BeautifulSoup`` 对象表示的是整个解析后的文档。
在绝大多数情况下，你可以直接把它当作 :ref:`Tag对象`，这意味着它支持 `在文档树中移动`_ 和 `对文档树进行搜索`_ 中描述的绝大多数方法。

你还可以对 ``BeautifulSoup`` 对象使用 `对文档树进行编辑`_ 提到的 ``.replace_with()`` 方法，用法和 :ref:`Tag对象` 类似。
这个方法可以让你完成一些操作，例如合并两个解析后的文档::
...
因为 ``BeautifulSoup`` 对象并不对应实际的HTML或XML文档中的tag，所以它没有name和attributes。
但有时如果能查看它的 ``.name`` 会有其他用处，因此我们给了它一个特殊的 ``.name`` ： "[document]"::

 soup.name
 # '[document]'

``Comment`` 对象（注释）和其他字符串
------------------------------------------

``Tag`` 、 ``NavigableString`` 和 ``BeautifulSoup`` 几乎涵盖了你会在HTML或XML文档中看到的所有对象，但是还有一小部分其他对象，
其中最主要的是 ``Comment`` 对象（即注释对象）::
...
当 ``Comment`` 对象作为HTML文档的一部分出现时，他会以特殊的样式输出::
...
Beautiful Soup还定义了 ``Stylesheet`` 、 ``Script`` 和 ``TemplateString`` 这几个类（class），
分别对应CSS样式表（``<style>`` 标签中的字符串）、Javascript（ ``<script>`` 标签中的字符串）以及HTML模板（ ``<template>`` 标签中的字符串）
这些类与 ``NavigableString`` 的特性和功能完全一致，定义它们只是为了方面地排除表示其他内容的字符串以提取网页的主体。
`(这些类仅适用于Beautiful Soup 4.9.0及以后版本，且不适用于html5lib解析器。)`
 
Beautiful Soup还定义一些类对应其他可能在XML文档中出现的对象：``CData`` 、 ``ProcessingInstruction`` 、
``Declaration`` 、 和 ``Doctype``。与 ``Comment`` 一样，它们都是 ``NavigableString`` 的子类，但是包含了一些其他信息。
这是一个把 ``Comment`` 对象替换为一个CDATA代码块的例子::
...
此处我们又用到了这个 “Three sisters” HTML文档::
...
下面我会以这篇文档为例向你展示如何从一篇文档的其中一处移动到另一处。

向下层移动
----------------

一个tag可能会包含字符串和其他tag，它们被称为原来那个tag的子结点（children)。
Beautiful Soup提供了许多属性让我们能在一个tag的子结点中实现移动和迭代。

请注意Beautiful Soup的 ``NavigableString`` 对象并不支持这些属性，因为HTML或XML文档中的字符串没有子结点。

使用tag的name属性移动
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

在文档树中最简单的移动方法是给出你想要移动到的tag的name。
例如想要移动到 <head> tag上，你只需要调用 ``soup.head``::
...
你可以不断地这样调用来到达文档树的特定位置。
下面这段代码可以让你移动到 <body>  tag的下面的第一个 <b> tag::

 soup.body.b
 # <b>The Dormouse's story</b>

但是这样像属性一样调用tag的name只能让你到达第一个对应name的tag::

 soup.a
 # <a class="sister" href="http://example.com/elsie" id="link1">Elsie</a>

如果你需要获得所有 <a> tag，或者需要进行其他复杂的操作，你就得使用 `对文档树进行搜索`_ 章节提到的方法，比如 ``.find_all()``::
...
通过调用 ``.contents`` ，你可以获得一个包含对应tag子结点的列表（list）::
...
顶层的 ``BeautifulSoup`` 对象也有子结点。在这个例子中，<html> tag就是 ``BeautifulSoup`` 对象的子结点::
...
你不能对 ``NavigableString`` 调用 ``.contents``，因为字符串没有子结点::

 text = title_tag.contents[0]
 text.contents
 # AttributeError: 'NavigableString' object has no attribute 'contents'

除了得到子结点列表，你可以调用 ``.children`` 得到一个生成器（generator）来迭代一个tag的子结点::
...
``.contents`` 和 ``.children`` 属性只返回tag的直接子结点（direct children），例如 <head> tag只有一个直接子结点 —— <title> tag::

 head_tag.contents
 # [<title>The Dormouse's story</title>]

但其实 <title> tag自己也有子结点：字符串 "The Dormouse's story"。从某种意义上讲，这个字符串也是 <head> tag的“子结点”。
使用 ``.descendants`` 方法则可以让你递归遍历一个tag的所有后代结点 [#]_ （descendants，或称为子孙节点)：它的直接子结点，它直接子结点的直接子结点，以此类推::
...
<head> tag 只有一个子结点，但是有两个后代结点： <title> tag 和它的子结点。
``BeautifulSoup`` 对象只有一个直接子结点（<html> tag），但是后代结点加起来有很多个::
...
如果一个tag只有一个子结点，并且那个子结点是一个 ``NavigableString`` ，你就可以用 ``.string`` 来获取它::

 title_tag.string
 # 'The Dormouse's story'

如果一个tag的唯的子结点是另一个可以调用 ``.string`` 的tag，那么它们被认为拥有相同的 ``.string`` 属性::
...
如果一个tag包含多个对象，且无法确定 ``.string`` 应该对应哪个对象的话，它的 ``.string`` 属性被定义为 ``None`` ::
...
如果一个tag里有多个对象，你可以通过 ``.strings`` 生成器来获得其中的字符串对象::
...
这些字符串通常会有一些多余的空白（whitespace） [#]_ ，你可以通过 ``.stripped_strings`` 生成器来获得去掉这些空白的字符串对象::
...
此处，所有只含空白的字符串都被忽略了，并且一个字符串首尾的空白也被移除::

向上层移动
-----------------

同样，每一个tag和字符串都有一个父结点（parent），即包含它的tag。
...
你可以调用 ``.parent`` 属性来获取一个元素的父结点。在 “three sisters” 文档中， <head> tag就是 <title> tag的父结点::
...
title字符串 “The Dormouse's story” 也有一个父结点，就是包含它的 <title> tag::

 title_tag.string.parent
 # <title>The Dormouse's story</title>

像 <html> 这样的顶层节点的父结点是 ``BeautifulSoup``  对象::

 html_tag = soup.html
 type(html_tag.parent)
 # <class 'bs4.BeautifulSoup'>

``BeautifulSoup``  对象的 ``.parent`` 属性被定义为 ``None``::
...
你可以通过 ``.parents`` 属性来迭代一个元素的父辈节点（parents）。
在这个例子中，我们使用 ``.parents`` 属性遍历了从一个相对底层的 <a> tag到文档顶层的所有结点::
...
这其中的 <b> tag和 <c> tag处在同一层级：他们是同一个tag的直接子结点。我们将其称为兄弟结点（siblings，或称同辈结点）。
如果你将一个文档漂亮打印（pretty-print），兄弟结点会拥有相同的缩进，你在写自己的文档时也可以运用这一规则，

``.next_sibling`` 和 ``.previous_sibling``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

你可以使用 ``.next_sibling`` 和 ``.previous_sibling`` 在文档树相同层级的元素之间移动::
...
<b> tag 有 ``.next_sibling`` 属性， 但是没有 ``.previous_sibling`` 属性，因为 <b> tag的前面没有它的同级元素了。
同样的，<c> tag 有 ``.previous_sibling`` 属性，但是没有 ``.next_sibling`` 属性::
...
"text1" 和 "text2" 两个字符串不是兄弟结点，因为他们的父结点不同::
...
在实际的文档中， 一个tag的 ``.next_sibling`` 或 ``.previous_sibling`` 属性往往是一个包含空白的字符串。
回到我们之前的 “three sisters” 文档::

 # <a href="http://example.com/elsie" class="sister" id="link1">Elsie</a>
 # <a href="http://example.com/lacie" class="sister" id="link2">Lacie</a>
 # <a href="http://example.com/tillie" class="sister" id="link3">Tillie</a>

你或许会认为第一个 <a> tag的  ``.next_sibling`` 属性会是第二个 <a> tag，
但它实际上是一个字符串：将第一个 <a> tag和第二个 <a> tag分开的“逗号和换行符”::
...
第二个 <a> tag 实际上是一个“逗号”的 ``.next_sibling``::
...
你可以通过 ``.next_siblings`` 或 ``.previous_siblings`` 来迭代一个tag的兄弟节点::
...
让我们来看一下 “three sisters” 文档的开头::

 # <html><head><title>The Dormouse's story</title></head>
 # <p class="title"><b>The Dormouse's story</b></p>

HTML解析器接收这一串字符后会进行如下操作：
...
Beautiful Soup提供了工具来复现如上的文档初始解析过程。
...
一个字符串或者tag的 ``.next_element`` 属性指向了紧随其后被解析的元素。
它可能与 ``.next_sibling`` 属性相同，但在大多数情况很不一样。

这是 “three sisters” 文档中的最后一个<a> tag。它的 ``.next_sibling`` 属性是一个字符串：被 <a> tag的结束标记打断的句子结尾 [#]_::
...
但是 <a> tag的 ``.next_element`` 属性，即紧随 <a> tag 后被解析的元素，并不是句子的剩余部分，而是 “Tillie” 这个词::

 last_a_tag.next_element
 # 'Tillie'

这是因为在原来的文档中， “Tillie” 这个词出现在分号前面。解析器的解析顺序是<a> tag、“Tillie” 这个单词、结束标记 </a> 、分号和句子的剩余部分。
此处的分号和 <a> tag 层级相同，但是 “Tillie” 这个单词先被解析。

``.previous_element`` 属性和 ``.next_element`` 属性正好相反，它指向正好在一个元素之前被解析的元素::
...
你或许能猜到这两个属性的用处了，你可以通过它们以被解析的顺序迭代文章中的元素::
...
Beautiful Soup定义了不少方法来对解析后生成的文档树进行搜索，但是他们都非常相似。
我会花很多时间讲解最常用的两个方法： ``.find()`` 和 ``.find_all()``，其他的方法接受的参数基本上与这两个方法相同，我只会简单的涉及。

我们还是使用 “three sisters” 文档作为例子::
...
通过给 ``.find_all()`` 等方法传入一个筛选器（filter，或称为过滤器），你可以聚焦到文档中你所感兴趣的部分。

筛选器的种类
----------------

在我们详细讲解 ``.find_all()`` 和其他类似的方法之前，我想要给你展示一些可以传入这些方法的筛选器的例子。
这些筛选器会整个搜索API部分反复出现，你可以使用它们进行基于tag的name或attributes、字符串的文本内容或者它们的某种组合的筛选。
...
.. rubric:: 尾注
.. [#] 为了和Python对象的属性区分开，原始XML或HTML文档中的tag的属性（attributes）的此处没有翻译，下同。
.. [#] 本篇文档并没有严格区分子结点（children）和后代结点（descendants）的称呼，有时子结点也指所有后代结点，请读者注意区分。
